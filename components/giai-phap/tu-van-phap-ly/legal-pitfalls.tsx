      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-[#fbbf24]/30 to-[#f59e42]/30 rounded-full px-6 py-2 border border-[#fbbf24]/40 mb-6 shadow-md">
            ...
            <span className="text-sm font-semibold text-[#b45309] tracking-wide">Cạm bẫy pháp lý thường gặp</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-extrabold text-[#0f172a] mb-4 drop-shadow-[0_2px_12px_rgba(251,191,36,0.10)]">
            Doanh nghiệp của bạn có đang vô tình bước vào những <span className="text-[#fbbf24]">"cạm bẫy"</span> này?
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-4xl mx-auto">
          {pitfalls.map((item, idx) => (
            <div
              key={idx}
              className={`group bg-white/80 rounded-3xl p-10 border-2 border-transparent hover:border-[#fbbf24] shadow-xl hover:shadow-2xl transition-all duration-300 flex gap-8 items-start relative overflow-hidden hover:scale-[1.03] hover:-translate-y-1`}
              style={{boxShadow:'0 8px 32px 0 rgba(251,191,36,0.10), 0 1.5px 8px 0 rgba(15,23,42,0.08)'}}
            >
              <div className={`w-20 h-20 bg-gradient-to-br ${item.color} rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300 border-4 border-white/60`}>
                <item.icon className="w-10 h-10 text-white drop-shadow-lg" />
              </div>
              <div>
                <h3 className="font-bold text-[#0f172a] mb-3 text-xl drop-shadow-sm">{item.title}</h3>
                <p className="text-slate-700 text-base leading-relaxed font-medium">{item.description}</p>
              </div>
              {/* Glow effect */}
