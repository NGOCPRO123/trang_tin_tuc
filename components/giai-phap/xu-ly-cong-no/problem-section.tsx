    ...
    <section className="py-20 bg-white/60 backdrop-blur-md">
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl lg:text-4xl font-black text-center text-[#CCA776] mb-4">
            Những Sai lầm Chết người khi Tự đi Đòi nợ
          </h2>
          <p className="text-xl text-center text-black font-bold mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
            {mistakes.map((item, idx) => (
              <div
                key={idx}
                className="flex items-start gap-4 bg-white rounded-xl p-6 border-l-4 border-[#CCA776] shadow-md hover:shadow-lg transition-all"
              >
                <div className="flex-shrink-0">
                  <div className="w-14 h-14 bg-[#CCA776]/30 rounded-full flex items-center justify-center">
                    <AlertTriangle className="w-7 h-7 text-[#CCA776]" />
                  </div>
                </div>
                <div>
                  <h3 className="font-black text-lg text-[#CCA776] mb-1 flex items-center">
                    <span className="mr-2">⚠️</span>{item.title}
                  </h3>
                  <p className="text-black text-base font-bold leading-relaxed">{item.description}</p>
                </div>
              </div>
            ))}
          </div>
