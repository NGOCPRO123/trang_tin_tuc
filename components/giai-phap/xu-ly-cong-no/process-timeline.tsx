          ...
          <Tabs defaultValue="0" className="w-full">
            <TabsList className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-8 bg-white/90">
              {steps.map((step, idx) => (
                <TabsTrigger key={idx} value={String(idx)} className="flex flex-col items-center text-xs md:text-sm">
                  <step.icon className="h-7 w-7 mb-1 text-red-600" />
                  Bước {idx + 1}
                </TabsTrigger>
              ))}
            </TabsList>
            {steps.map((step, idx) => (
              <TabsContent key={idx} value={String(idx)}>
                <div className="bg-white rounded-2xl shadow-xl border-l-4 border-red-400 p-8 md:p-12 flex flex-col md:flex-row gap-8 items-center">
                  <div className="flex-shrink-0 flex items-center justify-center w-20 h-20 rounded-full bg-red-100 mb-4 md:mb-0">
                    <step.icon className="h-10 w-10 text-red-600" />
                  </div>
                  <div>
                    <h3 className="text-xl font-bold mb-2 text-red-700">{step.title}</h3>
